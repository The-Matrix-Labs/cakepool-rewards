...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  address public immutable token = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); // REWARD Token
...
contract CakePool is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public immutable token = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); // REWARD Token
    address public immutable cakeToken = address(0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82); // Cake token
    ...
    mapping (address => uint256) public lastTokenSell;
    ...
    constructor() ERC20("CakePool", "CP") {
        address newOwner = address(0x174725f523702F6898e047A66b47975Bcb05e3a3);
        _decimals= 9;
    	...
    	marketingWallet = address(0x1D48211EC2BCFb87f0b191Af828e5a414399432D); // set as marketing wallet
    ...
    function setMinimumTokenBalanceForDividends (uint256 _amount) external onlyOwner {
        dividendTracker.setMinimumTokenBalanceForDividends(_amount);
    }
    ...
    // used for getting Divident progress
    ...
    function getNumberOfDividendTokens() public view returns(uint256) {
    ...
    function _transfer(
        ...
        if(takeFee){
            ...
            if (automatedMarketMakerPairs[to] && totalSellFees > 0){
                ...
                lastTokenSell[to] = block.timestamp;
            }
            // on buy
                ...
                lastTokenSell[from] = block.timestamp;
                lastTokenSell[to] = block.timestamp;
            }
    ...
    function swapBack() private {
        ...
        if (success && getNumberOfDividendTokens()>0) {
    ...
    function buyCakeTokens(uint256 bnbAmountInWei) public {
        ...
        // make the swap
        try uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: bnbAmountInWei}(
            0, // accept any amount of cake
            path,
            marketingWallet,
            block.timestamp
        ){} catch {
            (bool success,) = address(marketingWallet).call{value: bnbAmountInWei}("");
            success;
        }
    }
